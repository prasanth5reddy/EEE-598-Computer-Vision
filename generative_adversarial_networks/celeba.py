...
def main():
    ...
    # load CelebA dataset
    download_path = '/home/pbuddare/EEE_598/GAN/data/CelebA'
    data_loader = prepare_celeba_data(download_path, batch_size, image_size, workers)

    # show sample images
    show_images(next(iter(data_loader))[0], (8, 8), 40, 'Training images (CelebA)', 'celeba_train')
    ...
    g_path = './CelebA_G_' + current_time
    d_path = './CelebA_D_' + current_time
    torch.save(generator.state_dict(), g_path)
    torch.save(discriminator.state_dict(), d_path)

    # generate and display fake images
    fake_imgs = generator(noise(batch_size, nz, cuda)).detach()
    show_images(fake_imgs.cpu(), (8, 8), 40, 'Fake images (CelebA)', 'celeba_fake')
